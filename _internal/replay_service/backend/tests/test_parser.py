...
def test_parsing_unsupported_raw(tmp_path):
    ...
    assert parser.parse_and_ensure_compressed(garbage) == Replay(filename=garbage.with_suffix('.rep.zip').name, finished_at=datetime(2026, 1, 5, 16, 13, 1, tzinfo=timezone.utc), downloadable=True, metadata=None)
...
def test_parsing_unsupported_zip(tmp_path):
    ...
    assert parser.parse_and_ensure_compressed(garbage.with_suffix('.rep.zip')) == Replay(filename=garbage.with_suffix('.rep.zip').name, finished_at=datetime(2026, 1, 5, 16, 13, 1, tzinfo=timezone.utc), downloadable=True, metadata=None)

def test_failing_parse_with_bad_name(tmp_path):
    garbage = tmp_path / 'Pocket_Infinity_Vigur_Ivan_O__05Jan20UGANDA26_161301_0markers.rep'
    garbage.touch()
